...
from options import getGhidraMapOptions, loadOptions
...
def runHeadless(options, job, results, worker):
    ...
    while True:
        ...
        if injob.task == PROCESS_FILE:
            ...
            command.append(os.path.join("UEFISurveyor", "analyze_map.py"))
            command.append(filename.replace('.efi', '.map'))
...
if __name__ == "__main__":
    ...
    alloptions = loadOptions(os.path.join('UEFISurveyor', 'optionsMap.yaml'))
    options = getGhidraMapOptions(alloptions)
    proc_count = 16
    work = SimpleQueue()
    result = SimpleQueue()
    procs = []
    ...
    filelist = []
    for filename in os.listdir(options.efiPath):
